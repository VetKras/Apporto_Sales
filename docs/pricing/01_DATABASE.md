# Pricing Database Implementation — CoTutor + ExamSpace

Part of [00_INDEX.md](./00_INDEX.md). This file is the full Supabase schema
change: one migration, ready to drop into `supabase/migrations/`.

## Design decision: extend existing tables, don't fork a parallel schema

The indexer app's SQLite pipeline uses a generic cell-provenance schema (`pricing_sources` /
`pricing_values` / `pricing_formulas`) because its job is auditing arbitrary spreadsheet cells for an
AI assistant to cite. That's the wrong shape here — this app already has a purpose-built schema
(`pricing_config_versions` + `pricing_models`) that `pricing-engine.ts`, `AdminConfigTab.tsx`, and
`QuoteInputsPanel`/`QuoteOutputPanel` are already built around. Reuse it where it fits, extend it
minimally where it doesn't:

- **ExamSpace fits `pricing_models` as-is.** Six tiers, each a flat `$/student/year` rate with a cost
  and price — exactly the `tier_name` / `default_price` / `default_cost` shape that table already has.
  No new table needed, just new rows.
- **CoTutor does not fit `pricing_models`.** It's not a flat rate — it's a formula over live quote
  inputs (model choice, assignment volume, contract length). This needs two new small tables:
  assumptions (the "business levers" + "technical usage assumptions" from `BACKEND_ASSUMPTIONS`) and
  an approved AI model rate table (replaces the hardcoded `COTUTOR_MODELS` array).

## ⚠️ Before running this — check the existing `ai_models` table

Your bolt.new database view shows an `ai_models` table with **224 rows** that exists in Supabase but
appears in **no migration file in this repo** and is referenced by **no code in this repo**. I can't
inspect its columns from here. Before applying the migration below:

1. Open it in the Supabase table editor and check its columns.
2. If it already has something like `(model_id, input_rate, cached_input_rate, output_rate)` in
   `$/1M tokens`, it's probably a general model-pricing reference (likely too broad — 224 rows is far
   more than any product's approved list) — do **not** point the CoTutor dropdown at all 224 of them,
   that recreates the exact "unapproved model in the picker" problem this rebuild is fixing (the
   current app already has 4 fabricated Claude models in `COTUTOR_MODELS` that don't belong there).
   Consider adding an `is_cotutor_approved boolean` column to it instead of creating `cotutor_ai_models`
   from scratch, and skip creating that table below.
3. If its shape doesn't match, or its origin/purpose is unclear, proceed with `cotutor_ai_models` as
   written below — it's deliberately a small, curated, product-specific table so nothing this app shows
   sales reps can ever include an unapproved model again.

This migration is written assuming option 3 (create `cotutor_ai_models`). Adjust before applying if
option 2 turns out to be right.

## Migration file

`supabase/migrations/20260719000001_021_cotutor_examspace_pricing_v2.sql`
...
-- pricing_type stays 'per_student' (existing enum value, matches CoTutor/PowerGrader/TrustEd student-based
-- rows already in this table) — NOT 'per_seat_day'. unit becomes 'student/year'.
...
-- Not touched by this migration's formula work — copied as-is so the new active version still has a
-- full product catalog. Values remain the unsourced v1 placeholders; Phase 2 replaces these rows.
INSERT INTO pricing_models (config_version_id, product_id, tier_name, pricing_type, unit, default_price, currency, source_reference, confidence)
VALUES
...
## Why `pricing_models.default_cost` is populated for ExamSpace but wasn't before

The old seed rows (`010_seed_pricing.sql`) never set `default_cost` for ExamSpace — margin was never
visible in `AdminConfigTab`'s COGS column for this product. The new rows carry the real blended cost
from the Summary sheet, so `AdminConfigTab`'s existing `calcMargin()` display starts working correctly
for ExamSpace for the first time (61%–75% margin, increasing by tier, matching the workbook exactly —
nothing to change in that component, it already does the right math once given real cost data).

## What Phase 1 does NOT touch

`deals`, `quote_lines`, `quote_snapshots`, `products`, `competitors`, `competitive_matrix` — untouched.
`pricing_brackets` (Suite Tier) — untouched, still Phase 3/unresolved per the main plan.
